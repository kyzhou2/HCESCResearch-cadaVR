namespace VRTK.Examples
{
    ...
    public class UI_Interactions : MonoBehaviour
    {
        ...
        [SerializeField] private float maxLightIntensity = 1.13F;

        private const int EXISTING_CANVAS_COUNT = 4;
        private int activeBackdrop = 0;

        void Update()
        {
            foreach (Light l in lights)
            {
                l.intensity = slide.value * maxLightIntensity;
            }
        ...
        public void Dropdown(int value)
        {
            ...
            mc.activateModel(value);
        }

        public void modelDropDown(int value)
        {
            VRTK_Logger.Info("Dropdown option selected was ID " + value);
            mc.activateModel(value);
        }

        public void backdropDropdown(int value)
        {
            VRTK_Logger.Info("Dropdown option selected was ID " + value);
            backDrops[activeBackdrop].GetComponent<toggle>().tog();
            backDrops[value].GetComponent<toggle>().tog();
